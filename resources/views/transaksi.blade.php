          ...
          <h3><b>DATA PENJUALAN</b></h3>
            {{-- <ul class="nav nav-pills card-header-tabs">
              <li class="nav-item ms-2">
                <a href="/transaksi/tambah"><button type="button" class="btn btn-primary" style="background-color:#1a2075" > Tambah data </button></a>
              </li>
            </ul> --}}
          </div>
            <div class="card-body">
              ...
              <table class="table table-borderless table-hover align-middle mb-0 bg-white">
                ...
                <tbody>
                  <tr>
                    ...
                    @foreach($p->barang()->get() as $k)
                    <td>{{$k ->nama_barang}}</td>
                    <td class="text-success">Rp{{$k ->harga}}</td>
                    @endforeach
                    <td>{{ $p -> jumlah_beli}}</td>
                    <td>
                      <a href="/transaksi/edit/ {{ $p->id_transaksi }}" class="btn btn-link btn-sm text-primary">EDIT</a>
                      <a href="/transaksi/hapus/ {{ $p->id_transaksi }}" onclick="return confirm('Anda Yakin Menghapus Data Ini?')" class="btn btn-link btn-sm text-danger">HAPUS</i></a>
                    </td>
                  </tr>
                </tbody>
                @endforeach
